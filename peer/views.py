...
from account.models import Signup as sp
...
def peers_suggest_per_lang(userinfo):
    try:
        users = sp.objects.exclude(username = userinfo.username).filter(hobby__icontains= userinfo.hobby)
        rest_of_users = sp.objects.exclude(username = userinfo.username).exclude(hobby__icontains = userinfo.hobby)
